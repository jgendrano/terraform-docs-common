...
You can select to prioritize the values of variables in a variable set.
When a variable set is prioritized the values take precedence over any variables with the same key set at a more specific scope.
...
those with values set directly in the terraform configuration code itself.

If two prioritized variable sets include the same variable key, precedence is determined by the alphabetical order of the variable sets' names.
